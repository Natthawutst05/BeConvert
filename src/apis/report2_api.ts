...
export const fetchReport = async (): Promise<ResponseStandard> => {
  try {
    const response = await axios.get<ResponseStandard>(`${API_BASE_URL}/report2`);
...
export const fetchUpdate = async (): Promise<ResponseStandard> => {
  try {
    const response = await axios.get<ResponseStandard>(`${API_BASE_URL}/report2/updatedata`);
    return response.data;
  } catch (error: any) {
    console.error('Error fetching report data:', error);
...
export async function addReportStatus(payload: any) {
  try {
    const response = await axios.post(`${API_BASE_URL}/report2/add-status`, payload);
    return response.data;
  } catch (error) {
